...
struct DistributedVector
{
    ...
    updateBoundary()
    {
        ...
        //req[ 8] = mpiCart.comm.Isend(&grid(0,n), 1, MPI::DOUBLE, mpiCart.neighbors[NorthEast],  0);  
        //req[ 9] = mpiCart.comm.Isend(&grid(m,n), 1, MPI::DOUBLE, mpiCart.neighbors[SouthEast],  0);  
        //req[10] = mpiCart.comm.Isend(&grid(m,0), 1, MPI::DOUBLE, mpiCart.neighbors[SouthWest],  0);  
        //req[8] = mpiCart.comm.Isend(&grid(0,0), 1, MPI::DOUBLE, mpiCart.neighbors[NorthWest],  0);  

        //req[12] = mpiCart.comm.Irecv(&grid( -1,n+1), 1, MPI::DOUBLE, mpiCart.neighbors[NorthEast],  0);  
        //req[9] = mpiCart.comm.Irecv(&grid(m+1,n+1), 1, MPI::DOUBLE, mpiCart.neighbors[SouthEast],  0);  
        //req[14] = mpiCart.comm.Irecv(&grid(m+1, -1), 1, MPI::DOUBLE, mpiCart.neighbors[SouthWest],  0);  
        //req[15] = mpiCart.comm.Irecv(&grid( -1, -1), 1, MPI::DOUBLE, mpiCart.neighbors[NorthWest],  0);
...
main(int argc, char *argv[])
{
    ...
    int N = (1<<l) - 1;
    int n = (N+1)/2-1;
    DistributedVector f(mpiCart, N+1),
                      e(mpiCart, N+1),
                      u(mpiCart, N+1),
                      r(mpiCart, N+1);

    DistributedVector f_c(mpiCart, n+1),
                      u_c(mpiCart, n+1);
    ...
    DistributedVector::Grid rr, ff, uu, uuc;
    if ((mpiCart.row==0) && (mpiCart.col==0)) {
        read(rr, "r", mpiCart.numRows, mpiCart.numCols);
        read(ff, "f_c", mpiCart.numRows, mpiCart.numCols);
        read(uu, "u", mpiCart.numRows, mpiCart.numCols);
        read(uuc, "u_c", mpiCart.numRows, mpiCart.numCols);
        //cout << rr << endl;
        //cout << ff << endl;
        //cout << uu << endl;
        //cout << uuc << endl;
